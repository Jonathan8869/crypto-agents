...
def process_results(results: list):
    ...
    for result in results:
        ...
        if level == "HIGH":
            ...
            # Backtest: nur warnen, nie blockieren
            if bt_result.get("win_rate", 0) < 40 and bt_result.get("trades", 0) >= 10:
                result["confluence"]["details"].append(f"⚠️ Backtest warnt: Win-Rate nur {bt_result.get('win_rate',0)}%")
                print(f"[Coordinator] {result['symbol']} Backtest-Warnung (WR {bt_result.get('win_rate',0)}%)")
            ...
            if evaluation.get("approved"):
                ...
                if cash < position_size_usd:
                    ...
                    alert_agent.send_telegram(f"⚠️ Trade übersprungen: Nicht genug Cash (${cash:.2f})")
                else:
                    trade = open_paper_trade(
                        symbol=result["symbol"],
                        side=result.get("direction", "long").upper(),
                        entry_price=float(result.get("current_price") or result.get("price", 0)),
                    ...
                    if trade:
                        ...
                        if bt_result.get("trades", 0) > 0:
                            msg += f"\n\n{backtest.format_backtest(bt_result, compact=True)}"
                        # Portfolio-Status nach Trade anhängen
...
def eod_summary():
    ...
    msg += (
        ...
        f"P&L heute: ${pnl_sign}{status['total_pnl']:,.2f} ({pnl_sign}{status['total_return_pct']:.2f}%)\n"
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        msg = format_status_message(last_prices if last_prices else None)
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "  /close BTC - Position schliessen\n\n"
        "🔮 POLYMARKET\n"
        "  /poly - Sentiment Analyse\n"
        "  /poly_crypto - Crypto Markets\n"
        "  /poly_macro - Macro Markets\n\n"
        "📋 REPORTS\n"
        "  /briefing - Morning Briefing\n"
        "  /summary - Daily Summary\n"
        "  /alerts - Letzte Alerts\n"
...
async def cmd_trades(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Letzte abgeschlossene Trades mit P&L."""
    ...
    for t in history:
        ...
        lines.append(
            ...
            f"P&L: ${pnl:+.2f} ({pct:+.2f}%) | "
            f"{t.get('close_reason', '?')} | {closed_at}"
        )
...
async def cmd_close(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if symbol not in positions:
        await update.message.reply_text(f"❌ Keine offene Position für {symbol}")
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Aktuellen Portfolio-Status laden
    status = get_portfolio_status(last_prices if last_prices else None)
